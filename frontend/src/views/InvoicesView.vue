...
<template>
  <div>
    <WorkspaceStickyStack><header><div><p>Finance / receivables</p><h1>Invoices</h1><p>Invoice saved order snapshots once; derive receivables from allocations.</p></div><span>{{ rows.length }} invoices</span></header><section class="grid gap-3 sm:grid-cols-[minmax(0,1fr)_14rem_auto] sm:items-end"><SearchField v-model="query" label="Search invoices" placeholder="Search invoice, customer, or order"/><SelectField v-model="status" label="Status" :options="['All', 'Draft', 'Posted', 'Partially Paid', 'Paid', 'Voided'].map((value) => ({ label: value, value }))"/><span class="self-end pb-2">{{ filtered.length }} shown</span></section></WorkspaceStickyStack>
