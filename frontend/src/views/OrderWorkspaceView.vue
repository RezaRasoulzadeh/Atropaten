...
const props = withDefaults(defineProps<{ order: Order; isDraft?: boolean }>(), { isDraft: false })
...
const orderBalance = computed(() => props.order.total - props.order.paid)
const orderProfit = computed(() => props.order.items.reduce((total, item) => total + item.sellingPrice - item.estimatedCost, 0))
const orderMargin = computed(() => props.order.total ? Math.round((orderProfit.value / props.order.total) * 100) : 0)

const payments = [
  { date: '12 Aug 2024 · 09:04', method: 'Bank transfer', reference: 'TRX-882104', amount: 460, status: 'Allocated' },
  { date: 'Deposit requested', method: 'Pending', reference: '—', amount: 0, status: 'Not received' },
]
...
const history = [
  ...
  { date: '12 Aug · 09:04', title: 'Deposit recorded', detail: 'Bank transfer of $460 received and allocated.', tone: 'green' as BadgeTone },
...
function itemProfit(item: OrderItem) {
  return item.sellingPrice - item.estimatedCost
}

function itemMargin(item: OrderItem) {
  return item.sellingPrice ? Math.round((itemProfit(item) / item.sellingPrice) * 100) : 0
}
...
<template>
  <div class="order-workspace">
    <header class="order-workspace-header">
      <button class="back-button" type="button" @click="$emit('back')"><span aria-hidden="true">←</span> Orders</button>
      ...
      <div class="order-header-actions">
        <button class="button button-secondary" type="button" @click="notify('Order actions will be connected in a later milestone.')">More <span aria-hidden="true">⌄</span></button>
    ...
    <section v-if="activeTab === 'Overview'" class="order-tab-content">
      <div class="order-overview-grid">
        ...
        <SectionPanel title="Order summary" subtitle="Commercial totals and payment position" class="order-summary-panel">
          <div class="money-summary">
            <div><span class="money-label">Total</span><strong>{{ isDraft ? 'To be priced' : currency(order.total) }}</strong></div>
            <div><span class="money-label">Paid</span><strong class="value-positive">{{ isDraft ? '—' : currency(order.paid) }}</strong></div>
          ...
          <div v-if="!isDraft" class="payment-progress-block">
            <div class="payment-progress-label"><span>Paid to date</span><strong>{{ Math.round((order.paid / order.total) * 100) }}%</strong></div>
            <div class="payment-progress"><span :style="{ width: `${(order.paid / order.total) * 100}%` }"></span></div>
          </div>
          <div v-else class="draft-summary-note"><span class="callout-icon" aria-hidden="true">i</span><span>Prices will be calculated from the configured services in a later milestone.</span></div>
        </SectionPanel>
      ...
      <SectionPanel title="Order items" :subtitle="isDraft ? 'Add services from the Items tab.' : `${order.items.length} unrelated services in this order`" class="overview-items-panel">
        <div v-if="isDraft" class="overview-draft-items"><span class="empty-line-icon">+</span><span>No service lines yet.</span><button class="text-button" type="button" @click="activeTab = 'Items'">Add a service <span aria-hidden="true">→</span></button></div>
        <div v-else class="overview-item-list">
          <div v-for="item in order.items" :key="item.id" class="overview-item-row"><span class="item-index">{{ order.items.indexOf(item) + 1 }}</span><span class="overview-item-copy"><strong>{{ item.service }}</strong><small>{{ item.specification }}</small></span><span class="overview-item-qty">{{ item.quantity.toLocaleString() }} {{ item.unit }}</span><span class="overview-item-price">{{ currency(item.sellingPrice) }}</span><StatusBadge :label="item.productionStatus" :tone="productionTone(item.productionStatus)" /></div>
        </div>
      </SectionPanel>
    </section>
    ...
    <section v-else-if="activeTab === 'Items'" class="order-tab-content">
      <SectionPanel title="Order items" :subtitle="isDraft ? 'Compose unrelated services as separate line items.' : 'Each line keeps its own specification, cost, and production state.'" class="items-detail-panel">
        <template #action>
          <label class="margin-toggle"><input v-model="showMargins" type="checkbox" /> Show cost & margin</label>
        </template>
        <div v-if="isDraft" class="draft-items-list">
          <div v-for="(item, index) in draftItems" :key="item.id" class="draft-item-row">
            ...
            <button v-if="draftItems.length > 1" class="remove-item-button" type="button" aria-label="Remove item" @click="removeDraftItem(item.id)">×</button>
          </div>
          <div class="draft-items-footer"><button class="button button-secondary" type="button" @click="addDraftItem"><span class="button-icon" aria-hidden="true">+</span>Add service line</button><span class="draft-items-hint">A draft can contain multiple unrelated services.</span></div>
        </div>
        <div v-else class="item-cards">
          <article v-for="(item, index) in order.items" :key="item.id" class="item-card">
            ...
            <div class="item-card-metrics"><div><span>Quantity</span><strong>{{ item.quantity.toLocaleString() }} {{ item.unit }}</strong></div><div><span>Selling price</span><strong>{{ currency(item.sellingPrice) }}</strong></div><div v-if="showMargins"><span>Estimated cost</span><strong>{{ currency(item.estimatedCost) }}</strong></div><div v-if="showMargins"><span>Profit / margin</span><strong class="value-positive">{{ currency(itemProfit(item)) }} · {{ itemMargin(item) }}%</strong></div><button class="text-button" type="button" @click="notify(`Opening ${item.service} detail is available in the service configurator milestone.`)">Configure <span aria-hidden="true">→</span></button></div>
          </article>
        </div>
      </SectionPanel>
      ...
      <SectionPanel v-else title="Order profitability" subtitle="Cost visibility can be hidden when a customer is viewing the screen." class="profitability-panel">
        <div class="profitability-summary"><div><span>Estimated cost</span><strong>{{ currency(order.total - orderProfit) }}</strong></div><div><span>Profit</span><strong class="value-positive">{{ currency(orderProfit) }}</strong></div><div><span>Margin</span><strong class="value-positive">{{ orderMargin }}%</strong></div><button class="button button-secondary" type="button" @click="showMargins = !showMargins">{{ showMargins ? 'Hide margin' : 'Show margin' }}</button></div>
      </SectionPanel>
    </section>
    ...
    <section v-else-if="activeTab === 'Production'" class="order-tab-content">
      <SectionPanel title="Production plan" :subtitle="isDraft ? 'Production jobs appear after services are configured.' : 'Work sequencing follows each item promised date.'" class="production-workspace-panel">
        <div v-if="isDraft" class="tab-empty-state"><div class="empty-workspace-icon" aria-hidden="true">▣</div><h2>No production jobs yet</h2><p>Add and configure service lines first. The production queue will be created per item.</p><button class="button button-secondary" type="button" @click="activeTab = 'Items'">Go to items</button></div>
    ...
    <section v-else-if="activeTab === 'Payments'" class="order-tab-content">
      <div class="payment-tab-grid">
        <SectionPanel title="Payment position" subtitle="Deposits and payments against this order" class="payment-position-panel">
          <div class="money-summary"><div><span class="money-label">Order total</span><strong>{{ isDraft ? 'To be priced' : currency(order.total) }}</strong></div><div><span class="money-label">Paid</span><strong class="value-positive">{{ isDraft ? '—' : currency(order.paid) }}</strong></div><div><span class="money-label">Remaining</span><strong class="value-danger">{{ isDraft ? '—' : currency(orderBalance) }}</strong></div></div>
          <button class="button button-primary" type="button" @click="notify(isDraft ? 'Save the draft before recording a payment.' : 'Payment recording will be connected in the accounting milestone.')">Record payment</button>
        </SectionPanel>
      ...
      <SectionPanel title="Payment activity" subtitle="Mock transaction history for the order" class="payment-history-panel"><div v-if="isDraft" class="tab-empty-inline"><span class="empty-line-icon">—</span><span>No payment activity in a new draft.</span></div><div v-else class="payment-table-wrap"><table class="data-table payment-table"><thead><tr><th>Date</th><th>Method</th><th>Reference</th><th>Amount</th><th>Status</th></tr></thead><tbody><tr v-for="payment in payments" :key="`${payment.date}-${payment.method}`"><td>{{ payment.date }}</td><td class="table-primary">{{ payment.method }}</td><td>{{ payment.reference }}</td><td class="table-money">{{ valueOrDash(payment.amount) }}</td><td><StatusBadge :label="payment.status" :tone="payment.status === 'Allocated' ? 'green' : 'slate'" /></td></tr></tbody></table></div></SectionPanel>
    </section>
    ...
    <section v-else-if="activeTab === 'Files'" class="order-tab-content">
      <SectionPanel title="Order files" :subtitle="isDraft ? 'Attach artwork and proofs after the order is saved.' : 'Artwork, proofs, and customer-provided files'" class="files-panel">
        <div class="file-dropzone" :class="{ 'is-disabled': isDraft }"><span class="file-dropzone-icon" aria-hidden="true">↑</span><div><strong>{{ isDraft ? 'File attachments are available after saving' : 'Drop files here or choose from your computer' }}</strong><p>PDF, ZIP, PNG, JPG up to 50 MB</p></div><button class="button button-secondary" type="button" :disabled="isDraft" @click="notify('A system file picker will be connected in a later milestone.')">Choose files</button></div>
        <div v-if="isDraft" class="tab-empty-inline"><span class="empty-line-icon">—</span><span>No files attached to this draft.</span></div>
        <div v-else class="file-list"><div v-for="file in files" :key="file.name" class="file-row"><span class="file-type">{{ file.kind === 'Proof' ? 'PDF' : 'ZIP' }}</span><span class="file-copy"><strong>{{ file.name }}</strong><small>{{ file.detail }}</small></span><button class="text-button" type="button" @click="notify(`Preview for ${file.name} is not connected yet.`)">Preview</button><button class="icon-button" type="button" :aria-label="`More actions for ${file.name}`" @click="notify('File actions are coming soon.')">•••</button></div></div>
      </SectionPanel>
    </section>

    <section v-else class="order-tab-content">
      <SectionPanel title="Order history" subtitle="A chronological view of important changes" class="history-panel"><div v-if="isDraft" class="tab-empty-state"><div class="empty-workspace-icon" aria-hidden="true">↺</div><h2>No history yet</h2><p>Changes will appear here after the draft is saved.</p></div><div v-else class="history-timeline"><div v-for="event in history" :key="event.date" class="history-event"><span class="history-marker" :class="`tone-${event.tone}`" aria-hidden="true">•</span><div><span class="history-date">{{ event.date }}</span><h3>{{ event.title }}</h3><p>{{ event.detail }}</p></div></div></div></SectionPanel>
    </section>
  </div>
</template>
