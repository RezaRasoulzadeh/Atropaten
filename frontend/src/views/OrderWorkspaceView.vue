...
type HistoryEvent = { occurredAt: string; title: string; detail: string; amountRial?: number; tone: BadgeTone }
...
const payments = [
  { occurredAt: '2024-08-12T09:04:00+03:30', method: 'Bank transfer', reference: 'TRX-882104', amountRial: 46_000_000, status: 'Allocated' },
  { occurredAt: null, method: 'Pending', reference: '—', amountRial: 0, status: 'Not received' },
]

const files = [
  { name: 'mehr-studio-business-cards.pdf', size: '4.8 MB', addedAt: '2024-08-12', kind: 'Proof' },
  { name: 'menu-final-artwork.zip', size: '18.2 MB', addedAt: '2024-08-11', kind: 'Artwork' },
]

const history: HistoryEvent[] = [
  { occurredAt: '2024-08-12T10:22:00+03:30', title: 'Production started', detail: 'Business cards moved to the digital press queue.', tone: 'blue' as BadgeTone },
  { occurredAt: '2024-08-12T09:04:00+03:30', title: 'Deposit recorded', detail: 'Bank transfer received and allocated.', amountRial: 46_000_000, tone: 'green' as BadgeTone },
  { occurredAt: '2024-08-11T16:40:00+03:30', title: 'Order confirmed', detail: 'Commercial status changed from Quoted to Confirmed.', tone: 'blue' as BadgeTone },
  { occurredAt: '2024-08-11T14:18:00+03:30', title: 'Proof approved', detail: 'Mehr Studio approved the first proof by email.', tone: 'green' as BadgeTone },
]
  ...
  <div class="order-workspace">
    <div class="order-sticky-controls">
      ...
      <div class="order-meta-cell">
        ...
        <span class="meta-value" :class="{ 'text-danger': !isDraft && order.isOverdue }">{{ isDraft ? formatDraftPromised() : formatDateTime(order.promisedAt ?? '') }}</span>
      ...
      <div class="order-overview-grid">
        <SectionPanel title="Order details" :subtitle="isDraft ? 'Set the basics before adding services.' : 'Customer and delivery context'" class="order-details-panel">
          ...
          <div v-else class="detail-grid">
            ...
            <div><span class="detail-label">Created</span><strong>{{ formatDateTime(order.createdAt) }}</strong><span class="detail-muted">Order intake</span></div>
            <div><span class="detail-label">Promised date</span><strong :class="{ 'text-danger': order.isOverdue }">{{ formatDateTime(order.promisedAt ?? '') }}</strong><span class="detail-muted">Customer commitment</span></div>
    ...
    <section v-else-if="activeTab === 'Files'" class="order-tab-content">
      <SectionPanel title="Order files" :subtitle="isDraft ? 'Attach artwork and proofs after the order is saved.' : 'Artwork, proofs, and customer-provided files'" class="files-panel">
        ...
        <div v-else class="file-list"><div v-for="file in files" :key="file.name" class="file-row"><span class="file-type"><FileText :size="15" :stroke-width="1.8" aria-hidden="true" />{{ file.kind }}</span><span class="file-copy"><strong>{{ file.name }}</strong><small>{{ file.size }} · added {{ formatDate(file.addedAt) }}</small></span><button class="text-button" type="button" @click="notify(`Preview for ${file.name} is not connected yet.`)">Preview <ArrowRight :size="14" :stroke-width="1.8" aria-hidden="true" /></button><button class="icon-button" type="button" :aria-label="`More actions for ${file.name}`" @click="notify('File actions are coming soon.')"><MoreHorizontal :size="17" :stroke-width="1.8" aria-hidden="true" /></button></div></div>
      </SectionPanel>
    </section>

    <section v-else class="order-tab-content">
      <SectionPanel title="Order history" subtitle="A chronological view of important changes" class="history-panel"><div v-if="isDraft" class="tab-empty-state"><div class="empty-workspace-icon" aria-hidden="true"><HistoryIcon :size="21" :stroke-width="1.8" /></div><h2>No history yet</h2><p>Changes will appear here after the draft is saved.</p></div><div v-else class="history-timeline"><div v-for="event in history" :key="event.occurredAt" class="history-event"><span class="history-marker" :class="`tone-${event.tone}`" aria-hidden="true"><CheckCircle2 v-if="event.tone === 'green'" :size="15" :stroke-width="1.8" /><CircleAlert v-else :size="15" :stroke-width="1.8" /></span><div><span class="history-date">{{ formatDateTime(event.occurredAt) }}</span><h3>{{ event.title }}</h3><p>{{ event.detail }}<span v-if="event.amountRial"> {{ currency(event.amountRial) }}</span></p></div></div></div></SectionPanel>
    </section>
  </div>
</template>
