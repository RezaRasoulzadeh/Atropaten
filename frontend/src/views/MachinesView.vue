...
<template>
  <div>
    <WorkspaceStickyStack>
      ...
      <section aria-label="Machine filters"><label class="form-control gap-1"><span>Search machines</span><Search :size="16" :stroke-width="1.8" aria-hidden="true" /><input class="input input-bordered w-full min-w-0" v-model="query" type="search" placeholder="Search machine, code, or category" /></label><label class="form-control gap-1"><span>Status</span><span><select class="select select-bordered w-full min-w-0" v-model="filter" aria-label="Filter machines by status"><option>Active</option><option>Archived</option><option>All</option></select><ChevronDown :size="14" :stroke-width="1.8" aria-hidden="true" /></span></label><span>{{ filtered.length }} of {{ machines.length }} machines</span></section>
    </WorkspaceStickyStack>
