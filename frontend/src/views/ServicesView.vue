...
<template>
  <div>
    <WorkspaceStickyStack>
      ...
      <section class="grid gap-3 sm:grid-cols-[minmax(0,1fr)_12rem_auto] sm:items-end" aria-label="Service filters">
        <SearchField v-model="searchQuery" label="Search services" placeholder="Search service, code, or category" />
        <SelectField v-model="serviceFilter" label="Status" aria-label="Filter services by status" :options="['Active', 'Archived', 'All'].map((value) => ({ label: value, value }))" />
        <span class="self-end pb-2">{{ filteredServices.length }} of {{ services.length }} services</span>
      </section>
    </WorkspaceStickyStack>
        ...
        <form @submit.prevent="saveService">
          ...
          <div v-if="form.parameters.length">
            <article v-for="(parameter, index) in form.parameters" :key="parameter.id">
              ...
              <div><SelectField v-model="parameter.type" label="Type" :options="[{ label: 'Integer', value: 'integer' }, { label: 'Decimal', value: 'decimal' }, { label: 'Boolean', value: 'boolean' }, { label: 'Choice', value: 'choice' }, { label: 'Material reference', value: 'material-reference' }]" @update:model-value="normalizeParameter(parameter)"/><label class="form-control gap-1"><span>Required</span><span><input class="checkbox" v-model="parameter.required" type="checkbox" />Required input</span></label></div>
              ...
              <SelectField v-else-if="parameter.type === 'boolean'" v-model="parameter.defaultValue" label="Default value" :options="[{ label: 'Not set', value: '' }, { label: 'True', value: 'true' }, { label: 'False', value: 'false' }]" />
              <SelectField v-else-if="parameter.type === 'choice'" v-model="parameter.defaultValue" label="Default choice" :options="[{ label: 'Not set', value: '' }, ...parameter.options.map((option) => ({ label: option, value: option }))]" />
              <div v-else><SelectField v-model="parameter.defaultValue" label="Default material" :options="[{ label: 'No default material', value: '' }, ...materials.map((material) => ({ label: `${material.name}${material.sku ? ` · ${material.sku}` : ''}`, value: material.id }))]" /><small>Operators may select an active material later; consumption is not configured here.</small></div>
          ...
          <div v-if="form.components.length">
            <article v-for="(component, index) in form.components" :key="component.id">
              ...
              <div><label class="form-control gap-1"><span>Name</span><input class="input input-bordered w-full min-w-0" v-model="component.name" type="text" placeholder="Paper cost" /></label><SelectField v-model="component.type" label="Type" :options="[{ label: 'Material', value: 'material' }, { label: 'Machine', value: 'machine' }, { label: 'Labor', value: 'labor' }, { label: 'Outsourced', value: 'outsourced' }, { label: 'Fixed', value: 'fixed' }, { label: 'Overhead', value: 'overhead' }, { label: 'Waste', value: 'waste' }, { label: 'Manual', value: 'manual' }]" @update:model-value="updateComponentType(component)"/></div>
              ...
              <SelectField v-if="componentNeedsReference(component.type)" v-model="component.referenceId" :label="component.type === 'material' ? 'Material reference' : 'Machine reference'" :options="[{ label: `Select an active ${component.type}`, value: '' }, ...(component.type === 'material' ? materials : machines).map((item) => ({ label: item.name, value: item.id }))]"/>
              <div v-if="!componentNeedsPercentage(component.type)"><SelectField v-model="component.usageMode" label="Usage source" :options="[{ label: 'Fixed quantity', value: 'fixed' }, { label: 'Numeric parameter', value: 'parameter' }]"/><label class="form-control gap-1"><span>Fixed quantity</span><input class="input input-bordered w-full min-w-0" v-model="component.usageQuantity" type="text" inputmode="decimal" placeholder="1" /></label><label class="form-control gap-1"><span>Multiplier</span><input class="input input-bordered w-full min-w-0" v-model="component.multiplier" type="text" inputmode="decimal" placeholder="1" /></label></div>
              <label class="form-control gap-1" v-if="component.usageMode === 'parameter' && !componentNeedsPercentage(component.type)"><SelectField v-model="component.parameterKey" label="Usage parameter" :options="[{ label: 'Select numeric parameter', value: '' }, ...numericParameters().map((parameter) => ({ label: `${parameter.label || parameter.key} · ${parameter.key}`, value: parameter.key }))]"/><small v-if="component.parameterKey && !numericParameters().some((parameter) => parameter.key === component.parameterKey)">This reference is no longer numeric and will be rejected until corrected.</small></label>
              <div v-if="componentNeedsRate(component.type)"><label class="form-control gap-1"><span>Rate ({{ props.currencyUnit }})</span><input class="input input-bordered w-full min-w-0" v-model="component.rateInput" type="text" inputmode="decimal" placeholder="0" @input="updateComponentRate(component)" /></label><SelectField v-if="component.type === 'labor' || component.type === 'outsourced'" v-model="component.rateBasis" label="Rate basis" :options="[{ label: 'Per unit', value: 'unit' }, { label: 'Per minute', value: 'minute' }, { label: 'Per hour', value: 'hour' }]"/></div>
          ...
          <div v-if="form.pricingRule">
            <SelectField v-model="form.pricingRule.type" label="Rule" :options="[{ label: 'Manual / enter at quote time', value: 'manual' }, { label: 'Fixed selling price', value: 'fixed' }, { label: 'Cost plus markup %', value: 'markup' }, { label: 'Cost plus fixed margin', value: 'fixed-margin' }, { label: 'Per-unit parameter', value: 'per-unit' }, { label: 'Quantity tiers', value: 'quantity-tiers' }]" @update:model-value="normalizePricingRule"/>
            ...
            <template v-if="form.pricingRule.type === 'per-unit'"><SelectField v-model="form.pricingRule.parameterKey" label="Numeric parameter" :options="[{ label: 'Select parameter', value: '' }, ...numericParameters().map((parameter) => ({ label: parameter.label || parameter.key, value: parameter.key }))]"/><label class="form-control gap-1"><span>Rate / unit ({{ props.currencyUnit }})</span><input class="input input-bordered w-full min-w-0" :value="form.pricingRule.perUnitRateInput" inputmode="decimal" @input="updateGroupedMoney(form.pricingRule, 'perUnitRateInput', 'perUnitRateRial', $event)" /></label></template>
