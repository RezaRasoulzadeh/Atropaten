...
<template>
  <div>
    <WorkspaceStickyStack>
      <header>
        <div>
          <p>Catalog / sellable operations</p>
          <h1>Services</h1>
          <p>Define reusable work with operator-facing parameters for future pricing.</p>
        </div>
        <button class="btn btn-ghost" type="button" @click="startCreate"><Plus :size="16" :stroke-width="1.8" aria-hidden="true" />New service</button>
      </header>
      <section aria-label="Service filters">
        <label class="form-control gap-1"><span>Search services</span><Search :size="16" :stroke-width="1.8" aria-hidden="true" /><input class="input input-bordered w-full min-w-0" v-model="searchQuery" type="search" placeholder="Search service, code, or category" autocomplete="off" /></label>
        <label class="form-control gap-1"><span>Status</span><span><select class="select select-bordered w-full min-w-0" v-model="serviceFilter" aria-label="Filter services by status"><option>Active</option><option>Archived</option><option>All</option></select><ChevronDown :size="14" :stroke-width="1.8" aria-hidden="true" /></span></label>
    ...
    <section aria-label="Services workspace">
      <SectionPanel title="Service register" subtitle="Select a service to inspect its operator parameters.">
        ...
        <div v-else-if="filteredServices.length">
          <table class="table table-zebra w-full"><thead><tr><th scope="col">Service</th><th scope="col">Category</th><th scope="col">Parameters</th><th scope="col">Status</th><th scope="col">Updated</th></tr></thead>
            <tbody><tr v-for="service in filteredServices" :key="service.id" :class="{ 'bg-base-300': selectedId === service.id }" tabindex="0" @click="selectService(service.id)" @keydown.enter="selectService(service.id)"><td><span>{{ service.name }}</span><span>{{ service.code || 'No code' }}</span></td><td>{{ service.category || '—' }}</td><td><span>{{ service.parameters.length }} {{ service.parameters.length === 1 ? 'parameter' : 'parameters' }}</span><span>{{ service.parameters.filter((parameter) => parameter.required).length }} required</span></td><td><StatusBadge :label="service.active ? 'Active' : 'Archived'" :tone="service.active ? 'green' : 'slate'" /></td><td>{{ dateLabel(service.updatedAt) }}</td></tr></tbody>
          </table>
        </div>
        <div v-else><div aria-hidden="true"><SlidersHorizontal :size="21" :stroke-width="1.8" /></div><h2>{{ services.length ? 'No services match this view' : 'No services yet' }}</h2><p>{{ services.length ? 'Try another status or search term.' : 'Create the first reusable operation for your shop.' }}</p><button class="btn btn-ghost" v-if="!services.length" type="button" @click="startCreate"><Plus :size="15" :stroke-width="1.8" aria-hidden="true" />Create service</button></div>
      </SectionPanel>

      <SectionPanel v-if="editorMode" :title="editorMode === 'create' ? 'New service' : 'Edit service'" subtitle="The full definition saves atomically with its parameters.">
        <template #action><button class="btn btn-ghost" type="button" aria-label="Close service editor" @click="cancelEditor"><X :size="16" :stroke-width="1.8" aria-hidden="true" /></button></template>
        <form @submit.prevent="saveService">
          <div v-if="formError" role="alert">{{ formError }}</div>
          <label class="form-control gap-1"><span>Name</span><input class="input input-bordered w-full min-w-0" v-model="form.name" type="text" placeholder="Digital Print" autocomplete="off" /></label>
          <div><label class="form-control gap-1"><span>Code</span><input class="input input-bordered w-full min-w-0" v-model="form.code" type="text" placeholder="PRINT" autocomplete="off" /></label><label class="form-control gap-1"><span>Category</span><input class="input input-bordered w-full min-w-0" v-model="form.category" type="text" placeholder="Production" autocomplete="off" /></label></div>
          <label class="form-control gap-1"><span>Description / notes</span><textarea class="textarea textarea-bordered w-full min-w-0" v-model="form.description" rows="2" placeholder="What this operation covers"></textarea></label>
          <div><div><h3>Parameters</h3><p>Order is saved as shown and keys are stable references.</p></div><button class="btn btn-ghost" type="button" @click="addParameter"><ListPlus :size="15" :stroke-width="1.8" aria-hidden="true" />Add parameter</button></div>
          <div v-if="form.parameters.length">
            <article v-for="(parameter, index) in form.parameters" :key="parameter.id">
              <header><span>{{ String(index + 1).padStart(2, '0') }}</span><strong>{{ parameter.label || 'Untitled parameter' }}</strong><div><button class="btn btn-ghost" type="button" :disabled="index === 0" :aria-label="`Move ${parameter.label || 'parameter'} up`" @click="moveParameter(index, -1)"><ChevronUp :size="14" :stroke-width="1.8" aria-hidden="true" /></button><button class="btn btn-ghost" type="button" :disabled="index === form.parameters.length - 1" :aria-label="`Move ${parameter.label || 'parameter'} down`" @click="moveParameter(index, 1)"><ChevronDown :size="14" :stroke-width="1.8" aria-hidden="true" /></button><button class="btn btn-ghost" type="button" :aria-label="`Remove ${parameter.label || 'parameter'}`" @click="removeParameter(index)"><Trash2 :size="14" :stroke-width="1.8" aria-hidden="true" /></button></div></header>
              <div><label class="form-control gap-1"><span>Key</span><input class="input input-bordered w-full min-w-0" v-model="parameter.key" type="text" placeholder="paper_size" autocomplete="off" /></label><label class="form-control gap-1"><span>Label</span><input class="input input-bordered w-full min-w-0" v-model="parameter.label" type="text" placeholder="Paper size" autocomplete="off" /></label></div>
              <div><label class="form-control gap-1"><span>Type</span><select class="select select-bordered w-full min-w-0" v-model="parameter.type" @change="normalizeParameter(parameter)"><option value="integer">Integer</option><option value="decimal">Decimal</option><option value="boolean">Boolean</option><option value="choice">Choice</option><option value="material-reference">Material reference</option></select></label><label class="form-control gap-1"><span>Required</span><span><input class="checkbox" v-model="parameter.required" type="checkbox" />Required input</span></label></div>
              <div v-if="parameter.type === 'integer' || parameter.type === 'decimal'"><label class="form-control gap-1"><span>Minimum</span><input class="input input-bordered w-full min-w-0" v-model="parameter.minValue" type="text" inputmode="decimal" placeholder="Optional" /></label><label class="form-control gap-1"><span>Maximum</span><input class="input input-bordered w-full min-w-0" v-model="parameter.maxValue" type="text" inputmode="decimal" placeholder="Optional" /></label></div>
              <label class="form-control gap-1" v-if="parameter.type === 'integer' || parameter.type === 'decimal'"><span>Default value</span><input class="input input-bordered w-full min-w-0" v-model="parameter.defaultValue" type="text" inputmode="decimal" placeholder="Optional" /></label>
              <label class="form-control gap-1" v-else-if="parameter.type === 'boolean'"><span>Default value</span><select class="select select-bordered w-full min-w-0" v-model="parameter.defaultValue"><option value="">Not set</option><option value="true">True</option><option value="false">False</option></select></label>
              <label class="form-control gap-1" v-else-if="parameter.type === 'choice'"><span>Default choice</span><select class="select select-bordered w-full min-w-0" v-model="parameter.defaultValue"><option value="">Not set</option><option v-for="option in parameter.options" :key="option" :value="option">{{ option }}</option></select></label>
              <label class="form-control gap-1" v-else><span>Default material <em>optional</em></span><select class="select select-bordered w-full min-w-0" v-model="parameter.defaultValue"><option value="">No default material</option><option v-for="material in materials" :key="material.id" :value="material.id">{{ material.name }}{{ material.sku ? ` · ${material.sku}` : '' }}</option></select><small>Operators may select an active material later; consumption is not configured here.</small></label>
              <div v-if="parameter.type === 'choice'"><div><span>Choice options</span><button class="btn btn-ghost" type="button" @click="addOption(parameter)"><Plus :size="14" :stroke-width="1.8" aria-hidden="true" />Add option</button></div><div v-for="(option, optionIndex) in parameter.options" :key="`${parameter.id}-${optionIndex}`"><input class="input input-bordered w-full min-w-0" v-model="parameter.options[optionIndex]" type="text" :aria-label="`Choice option ${optionIndex + 1}`" placeholder="A4" @input="parameter.defaultValue = parameter.defaultValue === option ? parameter.options[optionIndex] : parameter.defaultValue" /><button class="btn btn-ghost" type="button" :aria-label="`Remove choice option ${optionIndex + 1}`" @click="removeOption(parameter, optionIndex)"><Trash2 :size="13" :stroke-width="1.8" aria-hidden="true" /></button></div><p v-if="!parameter.options.length">Add at least one option before saving.</p></div>
              <label class="form-control gap-1"><span>Unit / suffix <em>optional</em></span><input class="input input-bordered w-full min-w-0" v-model="parameter.unit" type="text" placeholder="sheets, hours, or mm" /></label>
            </article>
          </div>
          <div v-else><ListPlus :size="17" :stroke-width="1.8" aria-hidden="true" /><span>No parameters yet. Add one when the service needs operator input.</span></div>
          <div><div><h3>Cost components</h3><p>Ordered reusable inputs evaluated by the generic pricing preview.</p></div><button class="btn btn-ghost" type="button" @click="addComponent"><Plus :size="15" :stroke-width="1.8" aria-hidden="true" />Add component</button></div>
          <div v-if="form.components.length">
            <article v-for="(component, index) in form.components" :key="component.id">
              <header><span>{{ String(index + 1).padStart(2, '0') }}</span><strong>{{ component.name || 'Untitled component' }}</strong><div><button class="btn btn-ghost" type="button" :disabled="index === 0" :aria-label="`Move ${component.name || 'component'} up`" @click="moveComponent(index, -1)"><ChevronUp :size="14" :stroke-width="1.8" aria-hidden="true" /></button><button class="btn btn-ghost" type="button" :disabled="index === form.components.length - 1" :aria-label="`Move ${component.name || 'component'} down`" @click="moveComponent(index, 1)"><ChevronDown :size="14" :stroke-width="1.8" aria-hidden="true" /></button><button class="btn btn-ghost" type="button" :aria-label="`Remove ${component.name || 'component'}`" @click="removeComponent(index)"><Trash2 :size="14" :stroke-width="1.8" aria-hidden="true" /></button></div></header>
              <div><label class="form-control gap-1"><span>Name</span><input class="input input-bordered w-full min-w-0" v-model="component.name" type="text" placeholder="Paper cost" /></label><label class="form-control gap-1"><span>Type</span><span><select class="select select-bordered w-full min-w-0" v-model="component.type" @change="updateComponentType(component)"><option value="material">Material</option><option value="machine">Machine</option><option value="labor">Labor</option><option value="outsourced">Outsourced</option><option value="fixed">Fixed</option><option value="overhead">Overhead</option><option value="waste">Waste</option><option value="manual">Manual</option></select><ChevronDown :size="14" :stroke-width="1.8" aria-hidden="true" /></span></label></div>
              <label class="form-control gap-1"><input class="checkbox" v-model="component.enabled" type="checkbox" />Enabled for future pricing</label>
              <label class="form-control gap-1" v-if="componentNeedsReference(component.type)"><span>{{ component.type === 'material' ? 'Material reference' : 'Machine reference' }}</span><select class="select select-bordered w-full min-w-0" v-model="component.referenceId"><option value="">Select an active {{ component.type }}</option><option v-for="item in component.type === 'material' ? materials : machines" :key="item.id" :value="item.id">{{ item.name }}</option></select></label>
              <div v-if="!componentNeedsPercentage(component.type)"><label class="form-control gap-1"><span>Usage source</span><select class="select select-bordered w-full min-w-0" v-model="component.usageMode"><option value="fixed">Fixed quantity</option><option value="parameter">Numeric parameter</option></select></label><label class="form-control gap-1"><span>Fixed quantity</span><input class="input input-bordered w-full min-w-0" v-model="component.usageQuantity" type="text" inputmode="decimal" placeholder="1" /></label><label class="form-control gap-1"><span>Multiplier</span><input class="input input-bordered w-full min-w-0" v-model="component.multiplier" type="text" inputmode="decimal" placeholder="1" /></label></div>
              <label class="form-control gap-1" v-if="component.usageMode === 'parameter' && !componentNeedsPercentage(component.type)"><span>Usage parameter</span><select class="select select-bordered w-full min-w-0" v-model="component.parameterKey"><option value="">Select numeric parameter</option><option v-for="parameter in numericParameters()" :key="parameter.id" :value="parameter.key">{{ parameter.label || parameter.key }} · {{ parameter.key }}</option></select><small v-if="component.parameterKey && !numericParameters().some((parameter) => parameter.key === component.parameterKey)">This reference is no longer numeric and will be rejected until corrected.</small></label>
              <div v-if="componentNeedsRate(component.type)"><label class="form-control gap-1"><span>Rate ({{ props.currencyUnit }})</span><input class="input input-bordered w-full min-w-0" v-model="component.rateInput" type="text" inputmode="decimal" placeholder="0" @input="updateComponentRate(component)" /></label><label class="form-control gap-1" v-if="component.type === 'labor' || component.type === 'outsourced'"><span>Rate basis</span><select class="select select-bordered w-full min-w-0" v-model="component.rateBasis"><option value="unit">Per unit</option><option value="minute">Per minute</option><option value="hour">Per hour</option></select></label></div>
              <label class="form-control gap-1" v-if="componentNeedsPercentage(component.type)"><span>Percentage</span><input class="input input-bordered w-full min-w-0" v-model="component.percentage" type="text" inputmode="decimal" placeholder="10" /><small>Stored as an exact fixed-scale percentage of the applicable future cost basis.</small></label>
              <label class="form-control gap-1"><span>Notes <em>optional</em></span><input class="input input-bordered w-full min-w-0" v-model="component.notes" type="text" placeholder="Cost explanation or future basis" /></label>
            </article>
          </div>
          <div v-else><Plus :size="17" :stroke-width="1.8" aria-hidden="true" /><span>No cost components yet. Add reusable material, machine, labor, or other inputs.</span></div>
          <div><div><h3>Selling-price rule</h3><p>Choose a generic suggestion rule; operators can override it in the live configurator.</p></div></div>
          <div v-if="form.pricingRule">
            <label class="form-control gap-1"><span>Rule</span><select class="select select-bordered w-full min-w-0" v-model="form.pricingRule.type" @change="normalizePricingRule"><option value="manual">Manual / enter at quote time</option><option value="fixed">Fixed selling price</option><option value="markup">Cost plus markup %</option><option value="fixed-margin">Cost plus fixed margin</option><option value="per-unit">Per-unit parameter</option><option value="quantity-tiers">Quantity tiers</option></select></label>
            <label class="form-control gap-1" v-if="form.pricingRule.type === 'fixed'"><span>Fixed price ({{ props.currencyUnit }})</span><input class="input input-bordered w-full min-w-0" :value="form.pricingRule.fixedPriceInput" inputmode="decimal" @input="updateGroupedMoney(form.pricingRule, 'fixedPriceInput', 'fixedPriceRial', $event)" /></label>
            <label class="form-control gap-1" v-if="form.pricingRule.type === 'markup'"><span>Markup percentage</span><input class="input input-bordered w-full min-w-0" v-model="form.pricingRule.markupPercentage" inputmode="decimal" placeholder="20" /></label>
            <label class="form-control gap-1" v-if="form.pricingRule.type === 'fixed-margin'"><span>Fixed margin ({{ props.currencyUnit }})</span><input class="input input-bordered w-full min-w-0" :value="form.pricingRule.fixedMarginInput" inputmode="decimal" @input="updateGroupedMoney(form.pricingRule, 'fixedMarginInput', 'fixedMarginRial', $event)" /></label>
            <template v-if="form.pricingRule.type === 'per-unit'"><label class="form-control gap-1"><span>Numeric parameter</span><select class="select select-bordered w-full min-w-0" v-model="form.pricingRule.parameterKey"><option value="">Select parameter</option><option v-for="parameter in numericParameters()" :key="parameter.id" :value="parameter.key">{{ parameter.label || parameter.key }}</option></select></label><label class="form-control gap-1"><span>Rate / unit ({{ props.currencyUnit }})</span><input class="input input-bordered w-full min-w-0" :value="form.pricingRule.perUnitRateInput" inputmode="decimal" @input="updateGroupedMoney(form.pricingRule, 'perUnitRateInput', 'perUnitRateRial', $event)" /></label></template>
            <div v-if="form.pricingRule.type === 'quantity-tiers'"><div><span>Quantity tiers</span><button class="btn btn-ghost" type="button" @click="addPricingTier"><Plus :size="14" :stroke-width="1.8" />Add tier</button></div><div v-for="(tier, tierIndex) in form.pricingRule.tiers" :key="tier.position"><input class="input input-bordered w-full min-w-0" v-model="tier.minimumQuantity" inputmode="decimal" :placeholder="tierIndex === 0 ? '0' : '10'" :aria-label="`Tier ${tierIndex + 1} minimum quantity`" /><input class="input input-bordered w-full min-w-0" :value="tier.priceInput" inputmode="decimal" placeholder="Price" :aria-label="`Tier ${tierIndex + 1} price`" @input="updateGroupedMoney(tier, 'priceInput', 'priceRial', $event)" /><button class="btn btn-ghost" type="button" :aria-label="`Remove tier ${tierIndex + 1}`" @click="removePricingTier(tierIndex)"><Trash2 :size="13" :stroke-width="1.8" /></button></div><p v-if="!form.pricingRule.tiers.length">Add a zero-minimum tier before saving.</p></div>
            <p v-if="form.pricingRule.type === 'manual'">The live configurator will show cost and require an entered selling price.</p>
          </div>
          <div><button class="btn btn-ghost" type="button" @click="cancelEditor">Cancel</button><button class="btn btn-ghost" type="submit" :disabled="isSaving"><Save :size="15" :stroke-width="1.8" aria-hidden="true" />{{ isSaving ? 'Saving…' : 'Save service' }}</button></div>
        </form>
      </SectionPanel>

      <SectionPanel v-else-if="selectedService" title="Service inspector" subtitle="Current persisted definition">
        <template #action><button class="btn btn-ghost" type="button" aria-label="Edit selected service" @click="startEdit"><Edit3 :size="15" :stroke-width="1.8" aria-hidden="true" /></button></template>
        <div><StatusBadge :label="selectedService.active ? 'Active' : 'Archived'" :tone="selectedService.active ? 'green' : 'slate'" /><span>{{ selectedService.parameters.length }} parameters</span></div>
        <div><div><SlidersHorizontal :size="19" :stroke-width="1.8" aria-hidden="true" /></div><div><h3>{{ selectedService.name }}</h3><p>{{ selectedService.code || 'No code' }}<span v-if="selectedService.category"> · {{ selectedService.category }}</span></p></div></div>
        <p v-if="selectedService.description">{{ selectedService.description }}</p>
        <div><div v-for="parameter in selectedService.parameters" :key="parameter.id"><span>{{ String(parameter.position + 1).padStart(2, '0') }}</span><div><strong>{{ parameter.label }}</strong><small><code>{{ parameter.key }}</code> · {{ typeLabel(parameter.type) }}<span v-if="parameter.required"> · required</span></small><small v-if="parameter.type === 'choice'">{{ parameter.options.join(' / ') }}</small><small v-if="parameter.type === 'material-reference'">Active material selection</small><small v-if="parameter.defaultValue">Default: {{ parameter.type === 'material-reference' ? (materials.find((material) => material.id === parameter.defaultValue)?.name || parameter.defaultValue) : parameter.defaultValue }}</small></div></div><p v-if="!selectedService.parameters.length">No parameters configured.</p></div>
        <div><div>Cost components <span>{{ selectedService.components.length }}</span></div><div v-for="component in selectedService.components" :key="component.id"><span>{{ String(component.position + 1).padStart(2, '0') }}</span><div><strong>{{ component.name }}</strong><small>{{ componentSummary(component) }}</small></div></div><p v-if="!selectedService.components.length">No cost components configured.</p></div>
        <div>Updated {{ dateLabel(selectedService.updatedAt) }}</div>
        <div><button class="btn btn-ghost" v-if="selectedService.active" type="button" @click="setActive(false)"><Archive :size="15" :stroke-width="1.8" aria-hidden="true" />Archive</button><button class="btn btn-ghost" v-else type="button" @click="setActive(true)"><RotateCcw :size="15" :stroke-width="1.8" aria-hidden="true" />Reactivate</button></div>
      </SectionPanel>

      <SectionPanel v-else title="Service inspector" subtitle="Select a row to inspect it."><div><SlidersHorizontal :size="20" :stroke-width="1.8" aria-hidden="true" /><p>Service details will appear here.</p><button class="btn btn-ghost" type="button" @click="startCreate">Create a service <Plus :size="14" :stroke-width="1.8" aria-hidden="true" /></button></div></SectionPanel>
    </section>
