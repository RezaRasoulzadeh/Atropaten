...
<template>
  <div>
    <WorkspaceStickyStack>
      ...
      <section aria-label="Production filters"><label class="form-control gap-1"><span>Search production jobs</span><input class="input input-bordered w-full min-w-0" v-model="searchQuery" type="search" placeholder="Search job, order, customer, or service"/></label><label class="form-control gap-1"><span>Queue</span><span><select class="select select-bordered w-full min-w-0" v-model="statusFilter" aria-label="Production status" @change="load"><option>All</option><option>Pending</option><option>Ready</option><option>In Progress</option><option>Paused</option><option>Completed</option><option>Cancelled</option></select></span></label><span>{{ visibleJobs.length }} jobs</span></section>
    </WorkspaceStickyStack>
