...
<template>
  <div class="production-view">
    <WorkspaceStickyStack>
      ...
      <section class="production-filter-bar panel" aria-label="Production filters"><label class="production-search"><span class="sr-only">Search production jobs</span><input v-model="searchQuery" type="search" placeholder="Search job, order, customer, or service"/></label><label class="filter-control"><span>Queue</span><span class="select-control"><select v-model="statusFilter" aria-label="Production status" @change="load"><option>All</option><option>Pending</option><option>Ready</option><option>In Progress</option><option>Paused</option><option>Completed</option><option>Cancelled</option></select></span></label><span class="filter-result">{{ visibleJobs.length }} jobs</span></section>
    </WorkspaceStickyStack>
