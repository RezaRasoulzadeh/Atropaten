...
export interface LoanPayload {
  ...
  installments?: { id?: string; dueDate: string; principalRial: number; interestFeeRial: number }[]
}
...
export interface LoanPaymentPayload {
  ...
  principalRial: number
  interestRial: number
  allocations: { installmentId: string; principalRial: number; interestRial: number }[]
}
export const loansApi = {
  list(direction = '', status = '') {
    return ListLoans(direction, status) as unknown as Promise<LoanRecord[]>
  },
  get(id: string) {
    return GetLoan(id) as unknown as Promise<LoanRecord>
  },
