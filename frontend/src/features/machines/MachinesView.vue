...
const { page, pageSize, pageCount, pageNumbers, pagedItems: pagedMachines, goToPage } = useDynamicPagination(visibleMachines, { viewportSelector: '.machine-register-row' })
...
<template>
  ...
  <div v-else class="flex h-full min-h-0 min-w-0 flex-col overflow-hidden" :aria-label='$t("Machines workspace")'>
    <WorkspaceStickyStack class="shrink-0" :flush="true">
      <WorkspaceHeader :show-breadcrumb="true" :title='$t("Machines")' :description='$t("Keep reusable equipment rates ready for service cost definitions.")'>
        <SearchField v-model="searchQuery" class="w-full min-w-0 sm:w-64" :placeholder='$t("Search machines…")' :aria-label='$t("Search machines")' />
        <button class="btn btn-primary w-full gap-2 sm:w-auto" type="button" @click="startCreate"><Plus :size="16" aria-hidden="true" />{{ $t("Add machine") }}</button>
      </WorkspaceHeader>
    ...
    <div class="grid min-h-0 min-w-0 flex-1 grid-rows-[minmax(22rem,auto)_auto] gap-4 overflow-y-auto xl:grid-cols-[minmax(0,1.15fr)_minmax(24rem,0.85fr)] xl:grid-rows-1 xl:overflow-hidden">
      <section class="machine-register flex min-h-0 min-w-0 flex-col overflow-hidden rounded-box border border-base-300 bg-base-100" :aria-label='$t("Machine register")'>
        ...
        <div class="machine-register-table-head hidden grid-cols-[minmax(0,1.5fr)_minmax(7rem,0.8fr)_8rem_8rem_6rem_1.25rem] gap-3 border-b border-base-300 px-4 py-3 text-xs font-medium text-base-content/55"><span>{{ $t("Name") }}</span><span>{{ $t("Category") }}</span><span>{{ $t("Rate") }}</span><span>{{ $t("Basis") }}</span><span>{{ $t("Status") }}</span><span></span></div>
        <LoadingState v-if="isLoading" :label='$t("Loading machines…")' />
        <div v-else-if="pagedMachines.length" class="min-h-0 flex-1 overflow-y-auto divide-y divide-base-300">
          <button v-for="machine in pagedMachines" :key="machine.id" class="machine-register-row group grid w-full min-w-0 grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-3 px-4 py-3 text-start transition-colors hover:bg-base-200/60 focus-visible:bg-base-200/60 focus-visible:outline focus-visible:outline-1 focus-visible:outline-primary" :class="selectedId === machine.id ? 'bg-primary/10' : ''" type="button" @click="selectMachine(machine.id)">
            ...
            <span class="machine-register-desktop-only machine-register-basis hidden truncate text-xs text-base-content/70">{{ $ui(workspace.basisLabel(machine.rateBasis)) }}</span>
            <StatusBadge class="machine-register-desktop-only machine-register-status justify-self-end" :label="$ui(machine.active ? 'Active' : 'Archived')" :tone="machine.active ? 'green' : 'slate'" />
        ...
        <EmptyState v-else :title="$ui(machines.length ? 'No machines match this view' : 'No machines yet')" :description="$ui(machines.length ? 'Try another filter or search term.' : 'Add the first reusable rate input for production.')"><template #icon><Search :size="22" aria-hidden="true" /></template><template #action><button v-if="machines.length" class="btn btn-outline btn-sm" type="button" @click="clearFilters">{{ $t("Clear filters") }}</button><button v-else class="btn btn-primary btn-sm gap-2" type="button" @click="startCreate"><Plus :size="15" aria-hidden="true" />{{ $t("Create machine") }}</button></template></EmptyState>
        <footer class="flex shrink-0 flex-wrap items-center justify-between gap-3 border-t border-base-300 bg-base-100 px-4 py-3 text-xs text-base-content/60"><span>{{ $ui(visibleMachines.length ? `Showing ${(page - 1) * pageSize + 1}–${Math.min(page * pageSize, visibleMachines.length)} of ${visibleMachines.length} machines` : '0 machines') }}</span><div class="flex items-center gap-1"><button class="btn btn-ghost btn-xs btn-square" type="button" :disabled="page === 1" :aria-label='$t("Previous page")' @click="goToPage(page - 1)"><ChevronLeft :size="15" aria-hidden="true" /></button><button v-for="number in pageNumbers" :key="number" class="btn btn-xs min-w-8" :class="page === number ? 'btn-primary' : 'btn-ghost'" type="button" @click="goToPage(number)">{{ number }}</button><button class="btn btn-ghost btn-xs btn-square" type="button" :disabled="page === pageCount" :aria-label='$t("Next page")' @click="goToPage(page + 1)"><ChevronRight :size="15" aria-hidden="true" /></button></div></footer>
      </section>
      ...
      <section v-else class="flex min-h-72 min-w-0 items-center justify-center rounded-box border border-dashed border-base-300 p-8 text-center"><EmptyState :title='$t("Select a machine")' :description='$t("Choose a machine from the register to inspect rates and details.")'><template #icon><Layers3 :size="22" aria-hidden="true" /></template><template #action><button class="btn btn-primary btn-sm gap-2" type="button" @click="startCreate"><Plus :size="15" aria-hidden="true" />{{ $t("Create machine") }}</button></template></EmptyState></section>
    </div>
  </div>
</template>
