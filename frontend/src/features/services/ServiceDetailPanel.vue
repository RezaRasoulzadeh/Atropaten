  ...
  <section class="service-detail-panel h-auto min-h-0 min-w-0 overflow-visible rounded-box border border-base-300 bg-base-100 xl:h-full xl:overflow-y-auto" aria-label="Service details">
    <div class="border-b border-base-300 p-3 sm:p-5">
      ...
      <div class="mt-4 grid min-w-0 divide-y divide-base-300 border-y border-base-300 sm:mt-5 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
        <div class="flex items-center gap-3 py-3 sm:px-3 sm:first:pl-0"><Tag :size="20" class="shrink-0 text-primary" aria-hidden="true" /><div><span class="block text-xs text-base-content/55">Price preview</span><strong class="block text-sm">{{ priceSummary }}</strong></div></div>
    ...
    <div class="min-w-0 p-3 sm:p-4">
      <div v-if="activeTab === 'parameters'" class="space-y-2">
        <div class="grid min-w-0 gap-2 pb-2 sm:grid-cols-2">
          <div class="flex min-w-0 items-start gap-3 rounded-box border border-base-300 bg-base-200/20 px-3 py-2.5">
            ...
            <div class="min-w-0"><span class="block text-xs text-base-content/55">Material setup</span><strong class="block text-sm">{{ categoryRequirements.material ? 'Required' : 'Optional' }}</strong><small class="block truncate text-xs text-base-content/55">{{ materialComponentCount ? `${materialComponentCount} component${materialComponentCount === 1 ? '' : 's'} configured` : categoryRequirements.material ? 'Not configured' : 'Not needed by category' }}</small></div>
          </div>
          <div class="flex min-w-0 items-start gap-3 rounded-box border border-base-300 bg-base-200/20 px-3 py-2.5">
            ...
            <div class="min-w-0"><span class="block text-xs text-base-content/55">Machine setup</span><strong class="block text-sm">{{ categoryRequirements.machine ? 'Required' : 'Optional' }}</strong><small class="block truncate text-xs text-base-content/55">{{ machineComponentCount ? `${machineComponentCount} group${machineComponentCount === 1 ? '' : 's'} configured` : categoryRequirements.machine ? 'Not configured' : 'Not needed by category' }}</small></div>
          </div>
        </div>
        ...
        <div v-if="!parameters.length" class="rounded-box border border-dashed border-base-300 p-8 text-center text-sm text-base-content/60">No order inputs configured for this service.</div>
      </div>
      ...
      <div v-else-if="activeTab === 'components'" class="space-y-2">
        <div v-for="component in components" :key="component.id" class="flex min-w-0 items-center gap-3 rounded-box border border-base-300 bg-base-200/20 px-3 py-2.5">
          ...
          <span v-if="!component.enabled" class="badge badge-ghost shrink-0 text-xs">Disabled</span><span v-else-if="component.type !== 'overhead' && component.type !== 'waste' && componentAmount(component)" class="shrink-0 text-sm tabular-nums">{{ formatMoney(componentAmount(component), currencyUnit) }}</span>
        </div>
        <div v-if="!components.length" class="rounded-box border border-dashed border-base-300 p-8 text-center text-sm text-base-content/60">{{ categoryRequirements.material || categoryRequirements.machine ? 'Required cost setup is not configured yet.' : 'No cost components needed for this category.' }}</div>
      </div>
