...
<template>
  <section class="space-y-4 rounded-box border border-primary/35 bg-primary/5 p-4" :aria-label='$t("Print layout setup")'>
    ...
    <template v-if="enabled && form.finishedSize">
      ...
      <SelectField v-for="component in form.components.filter(c => c.type === 'machine')" :key="component.id" v-model="component.rateBasis" :label="$ui(`${component.name || 'Machine'} — apply selected rate per`)" :options="[{label: 'Configured usage / finished piece', value: ''}, {label: 'Consumed sheet',value:'sheet'}, {label: 'Consumed roll metre',value:'meter'}, {label: 'Consumed square metre (includes waste)',value:'square meter'}]" />
