...
<template>
  <section class="space-y-4 rounded-box border border-primary/35 bg-primary/5 p-4" aria-label="Print layout setup">
    <div class="flex items-center justify-between gap-3"><div><h3 class="font-semibold">Print size &amp; material layout</h3><p class="mt-1 text-xs text-base-content/60">Custom banners, roll rotation, waste, and multiple pieces per sheet.</p></div><span v-if="automaticRoll" class="badge badge-outline">Automatic for rolls</span><input v-else class="toggle toggle-primary" type="checkbox" aria-label="Calculate material layout" :checked="enabled" @change="($event.target as HTMLInputElement).checked ? enable() : disable()" /></div>
    <div v-if="!enabled" class="space-y-3">
      <p class="text-sm text-base-content/70">Enable layout to collect finished dimensions in orders and calculate material consumption for the entire quantity.</p>
      <button class="btn btn-primary btn-sm" type="button" @click="enable">Set up roll / sheet layout</button>
