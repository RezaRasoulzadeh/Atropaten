...
<template>
  ...
  <div v-else class="flex h-full min-h-0 min-w-0 flex-col overflow-hidden" :aria-label='$t("Services workspace")'>
    <WorkspaceStickyStack class="shrink-0" :flush="true">
      <WorkspaceHeader
        :show-breadcrumb="true"
        :title='$t("Services")'
        :description='$t("Manage reusable printing services, customer parameters, and pricing rules.")'
      >
        <SearchField v-model="searchQuery" class="w-full min-w-0 sm:w-64" :placeholder='$t("Search services…")' :aria-label='$t("Search services")' />
        <button class="btn btn-primary w-full gap-2 sm:w-auto" type="button" @click="startCreate"><Plus :size="16" aria-hidden="true" />{{ $t("Add service") }}</button>
      </WorkspaceHeader>
    ...
    <div class="grid min-h-0 min-w-0 flex-1 grid-rows-[minmax(22rem,auto)_auto] gap-4 overflow-y-auto xl:grid-cols-[minmax(0,1.15fr)_minmax(24rem,0.85fr)] xl:grid-rows-1 xl:overflow-hidden">
      <section class="flex min-h-0 min-w-0 flex-col overflow-hidden rounded-box border border-base-300 bg-base-100" :aria-label='$t("Service register")'>
        ...
        <div class="hidden grid-cols-[minmax(0,1.5fr)_minmax(7rem,0.8fr)_7.5rem_6rem_1.25rem] gap-3 border-b border-base-300 px-4 py-3 text-xs font-medium text-base-content/55 md:grid"><span>{{ $t("Name") }}</span><span>{{ $t("Category") }}</span><span>{{ $t("Last price") }}</span><span>{{ $t("Status") }}</span><span></span></div>
        <LoadingState v-if="isLoading" :label='$t("Loading services…")' />
        <div v-else-if="pagedServices.length" class="min-h-0 flex-1 overflow-y-auto divide-y divide-base-300">
          <button v-for="service in pagedServices" :key="service.id" class="group grid w-full min-w-0 grid-cols-[minmax(0,1fr)_auto_auto] items-center gap-3 px-4 py-3 text-start transition-colors hover:bg-base-200/60 focus-visible:bg-base-200/60 focus-visible:outline focus-visible:outline-1 focus-visible:outline-primary md:grid-cols-[minmax(0,1.5fr)_minmax(7rem,0.8fr)_7.5rem_6rem_1.25rem]" :class="selectedId === service.id ? 'bg-primary/10' : ''" type="button" @click="selectService(service.id)">
            <span class="flex min-w-0 items-center gap-3"><span class="size-9 shrink-0 overflow-hidden rounded-box border border-base-300 bg-base-200 bg-cover bg-center" :style="service.imagePath ? { backgroundImage: `url('${service.imagePath}')` } : undefined"><Layers3 v-if="!service.imagePath" :size="18" class="m-2 text-base-content/50" aria-hidden="true" /></span><span class="min-w-0"><strong class="block truncate text-sm">{{ service.name }}</strong><span class="block truncate text-xs text-base-content/60">{{ $ui(service.code || 'No code') }}<span v-if="service.description"> · {{ service.description }}</span></span></span></span>
            <span class="hidden truncate text-xs text-base-content/70 md:block">{{ $ui(service.category || 'Uncategorized') }}</span>
            <span class="hidden text-sm tabular-nums text-base-content/80 md:block">{{ $ui(estimatedPrice(service) !== null ? formatMoney(estimatedPrice(service) ?? 0, props.currencyUnit) : service.finishedSize?.quantityParameterKey ? 'By size & quantity' : 'Needs setup') }}</span>
            <StatusBadge class="justify-self-end md:justify-self-start" :label="$ui(service.active ? 'Active' : 'Archived')" :tone="service.active ? 'green' : 'slate'" />
            <ChevronRight :size="17" class="register-row-arrow justify-self-end text-base-content/45" aria-hidden="true" />
            <span class="col-span-3 flex flex-wrap gap-x-3 gap-y-1 text-xs text-base-content/55 md:hidden"><span>{{ $ui(service.category || 'Uncategorized') }}</span><span>{{ $ui(estimatedPrice(service) !== null ? formatMoney(estimatedPrice(service) ?? 0, props.currencyUnit) : service.finishedSize?.quantityParameterKey ? 'By size & quantity' : 'Needs setup') }}</span></span>
          </button>
        </div>
        <EmptyState v-else :title="$ui(services.length ? 'No services match this view' : 'No services yet')" :description="$ui(services.length ? 'Try another filter or search term.' : 'Create the first reusable operation for your shop.')">
          <template #icon><Search :size="22" aria-hidden="true" /></template>
          <template #action><button v-if="services.length" class="btn btn-outline btn-sm" type="button" @click="resetListFilters">{{ $t("Clear filters") }}</button><button v-else class="btn btn-primary btn-sm gap-2" type="button" @click="startCreate"><Plus :size="15" aria-hidden="true" />{{ $t("Create service") }}</button></template>
        </EmptyState>
        <footer class="flex shrink-0 flex-wrap items-center justify-between gap-3 border-t border-base-300 bg-base-100 px-4 py-3 text-xs text-base-content/60"><span>{{ $ui(visibleServices.length ? `Showing ${(page - 1) * pageSize + 1}–${Math.min(page * pageSize, visibleServices.length)} of ${visibleServices.length} services` : '0 services') }}</span><div class="flex items-center gap-1"><button class="btn btn-ghost btn-xs btn-square" type="button" :disabled="page === 1" :aria-label='$t("Previous page")' @click="goToPage(page - 1)"><ChevronLeft :size="15" aria-hidden="true" /></button><button v-for="number in pageNumbers" :key="number" class="btn btn-xs min-w-8" :class="page === number ? 'btn-primary' : 'btn-ghost'" type="button" @click="goToPage(number)">{{ number }}</button><button class="btn btn-ghost btn-xs btn-square" type="button" :disabled="page === pageCount" :aria-label='$t("Next page")' @click="goToPage(page + 1)"><ChevronRight :size="15" aria-hidden="true" /></button></div></footer>
      </section>
      ...
      <ServiceDetailPanel
        ...
        @remove="remove"
      />
      <section v-else class="flex min-h-72 min-w-0 items-center justify-center rounded-box border border-dashed border-base-300 p-8 text-center"><EmptyState :title='$t("Select a service")' :description='$t("Choose a service from the register to inspect its setup and pricing.")'><template #icon><Layers3 :size="22" aria-hidden="true" /></template></EmptyState></section>
    </div>
  </div>
</template>
