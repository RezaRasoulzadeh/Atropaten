...
const categoryOptions = computed(() => {
  const categories = [...new Set(services.value.map((service) => service.category).filter(Boolean))].sort((a, b) => a.localeCompare(b))
  return [{ label: 'All categories', value: '' }, ...categories.map((category) => ({ label: category, value: category }))]
})

const visibleServices = computed(() => {
  const items = filteredServices.value.filter((service) => !categoryFilter.value || service.category === categoryFilter.value)
...
watch([searchQuery, serviceFilter, categoryFilter, sortOrder], () => { page.value = 1 })
      ...
      <section class="flex min-h-0 min-w-0 flex-col overflow-hidden rounded-box border border-base-300 bg-base-100" aria-label="Service register">
        ...
        <div v-else-if="pagedServices.length" class="min-h-0 flex-1 overflow-y-auto divide-y divide-base-300">
          <button v-for="service in pagedServices" :key="service.id" class="group grid w-full min-w-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-4 py-3 text-start transition-colors hover:bg-base-200/60 focus-visible:bg-base-200/60 focus-visible:outline focus-visible:outline-1 focus-visible:outline-primary md:grid-cols-[minmax(0,1.5fr)_minmax(7rem,0.8fr)_7.5rem_6rem_1.25rem]" :class="selectedId === service.id ? 'bg-primary/10' : ''" type="button" @click="selectService(service.id)">
            ...
            <ChevronRight :size="17" class="hidden text-base-content/45 transition-transform group-hover:translate-x-0.5 md:block" aria-hidden="true" />
            <span class="col-span-2 flex flex-wrap gap-x-3 gap-y-1 text-xs text-base-content/55 md:hidden"><span>{{ service.category || 'Uncategorized' }}</span><span>{{ estimatedPrice(service) !== null ? formatMoney(estimatedPrice(service) ?? 0, props.currencyUnit) : 'Needs setup' }}</span></span>
          </button>
        </div>
