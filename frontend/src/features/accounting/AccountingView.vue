...
<div class="min-w-0 space-y-4">
  <WorkspaceStickyStack>
    <WorkspaceHeader
      ...
      :eyebrow='$t("Finance / ledger")'
      :title='$t("Accounting")'
      :description='$t("Balances and treasury accounts for the business.")'
    >
    </WorkspaceHeader>
    <WorkspaceTabs
  ...
  <LoadingState v-if="pageLoading" :label='$t("Loading records…")' />
  <div v-show="!pageLoading" class="min-w-0 space-y-4">
    <template v-if="tab === 'Overview'">
      <section class="grid min-w-0 gap-3 sm:grid-cols-2 xl:grid-cols-4" :aria-label='$t("Balance summary")'>
        <div class="flex min-h-32 min-w-0 flex-col rounded-box border border-base-300 bg-base-100 p-4">
          <div class="flex items-start justify-between gap-3">
            <p class="min-w-0 text-xs font-medium text-base-content/60">{{ $t("Total assets") }}</p>
          ...
          <p class="mt-auto pt-2 text-xs text-base-content/55">{{ $t("All asset accounts") }}</p>
        </div>
        <div class="flex min-h-32 min-w-0 flex-col rounded-box border border-base-300 bg-base-100 p-4">
          <div class="flex items-start justify-between gap-3">
            <p class="min-w-0 text-xs font-medium text-base-content/60">{{ $t("Cash on hand") }}</p>
          ...
          <p class="mt-auto pt-2 text-xs text-base-content/55">{{ $t("Cash financial account") }}</p>
        </div>
        <div class="flex min-h-32 min-w-0 flex-col rounded-box border border-base-300 bg-base-100 p-4">
          <div class="flex items-start justify-between gap-3">
            <p class="min-w-0 text-xs font-medium text-base-content/60">{{ $t("Receivable") }}</p>
          ...
          <p class="mt-auto pt-2 text-xs text-base-content/55">{{ $t("Outstanding customer balance") }}</p>
        </div>
        <div class="flex min-h-32 min-w-0 flex-col rounded-box border border-base-300 bg-base-100 p-4">
          <div class="flex items-start justify-between gap-3">
            <p class="min-w-0 text-xs font-medium text-base-content/60">{{ $t("Payable") }}</p>
          ...
          <p class="mt-auto pt-2 text-xs text-base-content/55">{{ $t("Outstanding supplier balance") }}</p>
        </div>
      </section>

      <section class="grid min-w-0 gap-4 xl:grid-cols-2">
        <AppPanel :title='$t("Financial snapshot")' :subtitle='$t("Current totals from the accounting balances.")'>
          <dl class="divide-y divide-base-300 rounded-box border border-base-300 bg-base-200/30">
            <div class="flex items-center justify-between gap-3 px-3 py-2.5 text-sm"><dt class="text-base-content/65">{{ $t("Bank balance") }}</dt><dd class="font-semibold tabular-nums">{{ money(bankBalance) }}</dd></div>
            <div class="flex items-center justify-between gap-3 px-3 py-2.5 text-sm"><dt class="text-base-content/65">{{ $t("Revenue") }}</dt><dd class="font-semibold tabular-nums text-success">{{ money(revenue) }}</dd></div>
            <div class="flex items-center justify-between gap-3 px-3 py-2.5 text-sm"><dt class="text-base-content/65">{{ $t("Expenses") }}</dt><dd class="font-semibold tabular-nums text-error">{{ money(expensesTotal) }}</dd></div>
          </dl>
          <p class="text-xs leading-5 text-base-content/55">{{ $t("Use Accounts to manage treasury accounts, or use the dedicated operational views for orders and invoices.") }}</p>
        </AppPanel>
        <AppPanel :title='$t("Recent transactions")' :subtitle='$t("The latest posted activity across the ledger.")' flush>
          ...
          <EmptyState v-else :title='$t("No recent transactions")' :description='$t("Posted accounting activity will appear here.")'><template #icon><BookOpen :size="21" aria-hidden="true" /></template></EmptyState>
        </AppPanel>
      </section>
    </template>

    <section v-else-if="tab === 'Accounts'" class="min-w-0 space-y-4">
      <AppPanel v-if="accountFormOpen" :title="$ui(editingAccount ? 'Edit treasury account' : 'Add treasury account')" :subtitle="$ui(editingAccount ? 'Update the account name, type, details, or attached owners.' : 'Create a cash or bank account for treasury movements.')">
        <form class="min-w-0 space-y-4" @submit.prevent="saveFinancialAccount">
          <section class="rounded-box border border-base-300 bg-base-200/20 p-3">
            <div class="mb-3"><h3 class="text-sm font-semibold">{{ $t("Account identity") }}</h3><p class="mt-0.5 text-xs text-base-content/55">{{ $t("Choose the account type and give it a clear name.") }}</p></div>
            <FormGrid :columns="2">
              <FormField :label='$t("Account name")'><AppInput v-model="accountForm.name" class="input w-full min-w-0" required :placeholder='$t("e.g. Main business bank")' /></FormField>
              <SelectField v-model="accountForm.type" :label='$t("Account type")' :options="[{ label: 'Bank account', value: 'bank' }, { label: 'Cash account', value: 'cash' }]" />
            </FormGrid>
          </section>
          <section v-if="accountForm.type === 'bank'" class="rounded-box border border-base-300 bg-base-200/20 p-3">
            <div class="mb-3"><h3 class="text-sm font-semibold">{{ $t("Bank details") }}</h3><p class="mt-0.5 text-xs text-base-content/55">{{ $t("Bank name is required; provide an account number or card number.") }}</p></div>
            <FormGrid :columns="3">
              <FormField :label='$t("Bank name")'><AppInput v-model="accountForm.bankName" class="input w-full min-w-0" required :placeholder='$t("e.g. Tejarat Bank")' /></FormField>
              <FormField :label='$t("Account number")'><AppInput v-model="accountForm.accountNumber" class="input w-full min-w-0" :placeholder='$t("Account number")' /></FormField>
              <FormField :label='$t("Card number")'><AppInput v-model="accountForm.cardNumber" class="input w-full min-w-0" inputmode="numeric" :placeholder='$t("Card number")' /></FormField>
            </FormGrid>
          </section>
          <section class="rounded-box border border-base-300 bg-base-200/20 p-3">
            <div class="mb-3"><h3 class="text-sm font-semibold">{{ $t("Additional information") }}</h3><p class="mt-0.5 text-xs text-base-content/55">{{ $t("Keep optional notes and ownership in one place.") }}</p></div>
            <div class="space-y-4">
              <FormField :label='$t("Details")'><AppTextarea v-model="accountForm.details" rows="3" :placeholder='$t("Branch, purpose, or other notes")' /></FormField>
              <FormField :label='$t("Attached owners")'>
                ...
                <EmptyState v-else compact :title='$t("No owners available")' :description='$t("Add owners from the Owners workspace to attach them here.")'><template #icon><Users :size="21" aria-hidden="true" /></template></EmptyState>
              </FormField>
            </div>
          </section>
          <div class="flex flex-wrap justify-end gap-2">
            <button class="btn btn-ghost" type="button" @click="cancelAccountEdit">{{ $t("Cancel") }}</button>
            <button class="btn btn-primary" type="submit" :disabled="busy"><Save :size="15" /> {{ $ui(editingAccount ? 'Save changes' : 'Add account') }}</button>
          </div>
        </form>
      </AppPanel>
      <AppPanel :title='$t("Treasury accounts")' :subtitle='$t("Cash and bank accounts available to payments, expenses, transfers, checks, loans, and owner movements.")' flush>
        <template #action><button class="btn btn-primary btn-sm" type="button" @click="startAccount()"><Plus :size="15" /> {{ $t("New account") }}</button></template>
        <DataTable v-if="financial.length">
          <thead><tr><th>{{ $t("Account") }}</th><th>{{ $t("Type") }}</th><th>{{ $t("Owners") }}</th><th class="text-end">{{ $t("Balance") }}</th><th>{{ $t("Status") }}</th><th class="text-end">{{ $t("Actions") }}</th></tr></thead>
          <tbody>
            <tr v-for="account in financial" :key="account.id">
              <DataTableCell><strong class="block">{{ account.name }}</strong><span class="block max-w-72 truncate text-xs text-base-content/60">{{ $ui(account.type === 'bank' ? [account.bankName, account.accountNumber && `Account ${account.accountNumber}`, account.cardNumber && `Card ${account.cardNumber}`].filter(Boolean).join(' · ') || 'Bank details not set' : account.details || 'Cash account') }}</span></DataTableCell>
              <DataTableCell><StatusBadge :label="$ui(account.type === 'cash' ? 'Cash' : 'Bank')" :tone="account.type === 'cash' ? 'green' : 'blue'" /></DataTableCell>
              <DataTableCell><span v-if="ownerNames(account.ownerIds).length" class="text-xs">{{ $ui(ownerNames(account.ownerIds).join(' · ')) }}</span><span v-else class="text-xs text-base-content/50">{{ $t("Business account") }}</span></DataTableCell>
              <DataTableCell numeric>{{ money(account.balanceRial) }}</DataTableCell>
              <DataTableCell><StatusBadge :label="$ui(account.active ? 'Active' : 'Archived')" :tone="account.active ? 'green' : 'slate'" /></DataTableCell>
              <DataTableCell><div class="flex justify-end gap-2"><button class="btn btn-outline btn-info btn-sm" type="button" @click="startAccount(account)"><Pencil :size="14" /> {{ $t("Edit") }}</button><button v-if="account.active" class="btn btn-outline btn-warning btn-sm" type="button" @click="archiveFinancialAccount(account)"><Archive :size="14" /> {{ $t("Archive") }}</button><button v-if="account.id !== 'FIN-CASH'" class="btn btn-outline btn-error btn-sm" type="button" @click="removeFinancialAccount(account)"><Trash2 :size="14" /> {{ $t("Delete") }}</button></div></DataTableCell>
            </tr>
          </tbody>
        </DataTable>
        <EmptyState v-else :title='$t("No treasury accounts")' :description='$t("Add a cash or bank account to start tracking treasury activity.")'>
          <template #icon><Landmark :size="22" aria-hidden="true" /></template>
          <template #action><button class="btn btn-primary btn-sm gap-2" type="button" @click="startAccount()"><Plus :size="15" aria-hidden="true" /> {{ $t("Create account") }}</button></template>
        </EmptyState>
      </AppPanel>
    </section>
