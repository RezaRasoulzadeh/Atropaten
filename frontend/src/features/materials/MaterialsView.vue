...
<template>
  ...
  <div v-else class="flex h-full min-h-0 min-w-0 flex-col overflow-hidden" :aria-label='$t("Materials workspace")'>
    <WorkspaceStickyStack class="shrink-0" :flush="true">
      <WorkspaceHeader :show-breadcrumb="true" :title='$t("Materials")' :description='$t("Keep physical stock, conversion units, and cost basis ready for production.")'>
        <SearchField v-model="searchQuery" class="w-full min-w-0 sm:w-64" :placeholder='$t("Search materials…")' :aria-label='$t("Search materials")' />
        <button class="btn btn-primary w-full gap-2 sm:w-auto" type="button" @click="startCreate"><Plus :size="16" aria-hidden="true" />{{ $t("Add material") }}</button>
      </WorkspaceHeader>
    ...
    <div class="grid min-h-0 min-w-0 flex-1 grid-rows-[minmax(22rem,auto)_auto] gap-4 overflow-y-auto xl:grid-cols-[minmax(0,1.15fr)_minmax(24rem,0.85fr)] xl:grid-rows-1 xl:overflow-hidden">
      <section class="material-register flex min-h-0 min-w-0 flex-col overflow-hidden rounded-box border border-base-300 bg-base-100" :aria-label='$t("Material register")'>
        ...
        <div class="material-register-table-head hidden grid-cols-[minmax(0,1.5fr)_minmax(7rem,0.8fr)_8rem_7rem_6rem_1.25rem] gap-3 border-b border-base-300 px-4 py-3 text-xs font-medium text-base-content/55"><span>{{ $t("Name") }}</span><span>{{ $t("Kind") }}</span><span>{{ $t("Unit cost") }}</span><span>{{ $t("Stock") }}</span><span>{{ $t("Status") }}</span><span></span></div>
        <LoadingState v-if="isLoading" :label='$t("Loading materials…")' />
        <div v-else-if="pagedMaterials.length" class="min-h-0 flex-1 overflow-y-auto divide-y divide-base-300">
          <button v-for="material in pagedMaterials" :key="material.id" class="material-register-row group grid w-full min-w-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-4 py-3 text-start transition-colors hover:bg-base-200/60 focus-visible:bg-base-200/60 focus-visible:outline focus-visible:outline-1 focus-visible:outline-primary" :class="selectedId === material.id ? 'bg-primary/10' : ''" type="button" @click="selectMaterial(material.id)">
            <span class="material-register-mobile-card">
              ...
              <span class="material-register-mobile-identity min-w-0 self-center"><strong class="block truncate text-sm">{{ material.name }}</strong><span class="block truncate text-xs text-base-content/60">{{ $ui(material.sku || 'No SKU') }}<span v-if="material.consumptionUnit"> · {{ $ui(material.consumptionUnit) }}</span></span></span>
              <StatusBadge class="material-register-mobile-status justify-self-end self-center" :label="$ui(material.active ? 'Active' : 'Archived')" :tone="material.active ? 'green' : 'slate'" />
              ...
              <span class="material-register-mobile-summary flex min-w-0 flex-wrap gap-x-3 gap-y-1 text-xs text-base-content/55"><span>{{ $ui(kindLabel(material.kind)) }}</span><span>{{ formatMoney(unitCost(material), props.currencyUnit) }} / {{ $ui(material.consumptionUnit) }}</span><span>{{ material.availableStock }} {{ $t("available") }}</span></span>
            </span>
            <span class="material-register-name material-register-desktop-only flex min-w-0 items-center gap-3"><span class="grid size-9 shrink-0 place-items-center rounded-box border border-base-300 bg-base-200 text-primary"><Package :size="18" aria-hidden="true" /></span><span class="min-w-0"><strong class="block truncate text-sm">{{ material.name }}</strong><span class="block truncate text-xs text-base-content/60">{{ $ui(material.sku || 'No SKU') }}<span v-if="material.consumptionUnit"> · {{ $ui(material.consumptionUnit) }}</span></span></span></span>
            <span class="material-register-category material-register-desktop-only hidden truncate text-xs text-base-content/70">{{ $ui(kindLabel(material.kind)) }}</span>
            ...
            <span class="material-register-stock material-register-desktop-only hidden truncate text-xs text-base-content/70">{{ material.availableStock }} {{ $ui(material.consumptionUnit) }}</span>
            <StatusBadge class="material-register-status material-register-desktop-only justify-self-end" :label="$ui(material.active ? 'Active' : 'Archived')" :tone="material.active ? 'green' : 'slate'" />
        ...
        <EmptyState v-else :title="$ui(materials.length ? 'No materials match this view' : 'No materials yet')" :description="$ui(materials.length ? 'Try another filter or search term.' : 'Create the first material to establish a production stock baseline.')"><template #icon><Search :size="22" aria-hidden="true" /></template><template #action><button v-if="materials.length" class="btn btn-outline btn-sm" type="button" @click="clearFilters">{{ $t("Clear filters") }}</button><button v-else class="btn btn-primary btn-sm gap-2" type="button" @click="startCreate"><Plus :size="15" aria-hidden="true" />{{ $t("Create material") }}</button></template></EmptyState>
        <footer class="flex shrink-0 flex-wrap items-center justify-between gap-3 border-t border-base-300 bg-base-100 px-4 py-3 text-xs text-base-content/60"><span>{{ $ui(visibleMaterials.length ? `Showing ${(page - 1) * pageSize + 1}–${Math.min(page * pageSize, visibleMaterials.length)} of ${visibleMaterials.length} materials` : '0 materials') }}</span><div class="flex items-center gap-1"><button class="btn btn-ghost btn-xs btn-square" type="button" :disabled="page === 1" :aria-label='$t("Previous page")' @click="goToPage(page - 1)"><ChevronLeft :size="15" aria-hidden="true" /></button><button v-for="number in pageNumbers" :key="number" class="btn btn-xs min-w-8" :class="page === number ? 'btn-primary' : 'btn-ghost'" type="button" @click="goToPage(number)">{{ number }}</button><button class="btn btn-ghost btn-xs btn-square" type="button" :disabled="page === pageCount" :aria-label='$t("Next page")' @click="goToPage(page + 1)"><ChevronRight :size="15" aria-hidden="true" /></button></div></footer>
      </section>

      <MaterialDetailPanel v-if="selectedMaterial" :workspace="workspace" :currency-unit="props.currencyUnit" />
      <section v-else class="flex min-h-72 min-w-0 items-center justify-center rounded-box border border-dashed border-base-300 p-8 text-center"><EmptyState :title='$t("Select a material")' :description='$t("Choose a material from the register to inspect stock and cost details.")'><template #icon><Package :size="22" aria-hidden="true" /></template></EmptyState></section>
    </div>
  </div>
</template>
