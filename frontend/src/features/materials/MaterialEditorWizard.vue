...
const skuAutoGenerated = ref(false)
...
function syncSkuFromName() {
  if (!skuAutoGenerated.value) return
  const generated = skuFromName(form.value.name)
  if (!generated) return
  if (!form.value.sku.trim() || form.value.sku === lastGeneratedSku.value) {
    form.value.sku = generated
    lastGeneratedSku.value = generated
  }
}

function onSkuInput(value: string) {
  skuAutoGenerated.value = false
  form.value.sku = value
}

watch(() => editorMode.value, () => {
  skuAutoGenerated.value = !form.value.sku.trim()
  lastGeneratedSku.value = form.value.sku
  syncSkuFromName()
}, { immediate: true })
watch(() => form.value.name, syncSkuFromName)
...
function updateKind(kind: string) {
  ...
  if (editorMode.value === 'create' && (kind === 'roll-media' || kind === 'fabric')) {
    form.value.purchaseUnit = 'meter'
    form.value.consumptionUnit = 'meter'
                ...
                <div v-for="group in specificationGroups" :key="group.name" class="space-y-3">
                  ...
                  <div class="grid min-w-0 gap-4 sm:grid-cols-2">
                    <FormField v-for="definition in group.definitions" :key="definition.key" class="gap-1">
                      ...
                      <SelectField v-if="definition.valueType === 'enum'" :model-value="String(attributeValue(definition.key))" :options="enumOptions(definition)" :aria-label="$ui(definition.label)" @update:model-value="setAttribute(definition.key, definition.valueType, $event)" />
                      <div v-else-if="definition.valueType === 'boolean'" class="flex h-10 items-center gap-2"><input class="checkbox" type="checkbox" :checked="Boolean(attributeValue(definition.key))" :aria-label="$ui(definition.label)" @change="setAttribute(definition.key, definition.valueType, ($event.target as HTMLInputElement).checked)" /><span class="text-sm">{{ $t("Enabled") }}</span></div>
