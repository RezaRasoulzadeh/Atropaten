...
<template>
  <div class="service-wizard w-full flex min-h-0 min-w-0 flex-col gap-4 overflow-visible xl:h-full xl:overflow-hidden" :aria-label='$t("Production job editor")'>
      ...
      <div class="grid min-h-0 min-w-0 flex-1 gap-4 overflow-visible xl:grid-cols-[minmax(0,1fr)_20rem] xl:overflow-hidden">
        <section class="service-wizard-form-panel min-h-0 min-w-0 rounded-box border border-base-300 bg-base-200/20 p-4 sm:p-6 xl:overflow-y-auto">
          <form id="production-editor-wizard" class="service-editor min-w-0" @submit.prevent="submit">
            <section v-if="activeStep === 1" class="min-w-0 space-y-6"><div class="flex items-center gap-3 border-b border-base-300 pb-4"><span class="grid size-10 shrink-0 place-items-center rounded-box bg-primary/10 text-primary"><ClipboardList :size="21" aria-hidden="true" /></span><div><h2 class="text-lg font-semibold">{{ $ui(isCreating ? 'Choose the order' : 'Job context') }}</h2><p class="text-sm text-base-content/60">{{ $ui(isCreating ? 'Production jobs can only be created from confirmed orders.' : 'The order, service, and item snapshot are protected after creation.') }}</p></div></div><template v-if="isCreating"><SelectField v-model="form.orderId" :label='$t("Confirmed order")' :aria-label='$t("Confirmed order")' :options="orderOptions" :invalid="validationAttempted && (!form.orderId || !confirmedOrders.some((order) => order.id === form.orderId))" @update:model-value="chooseOrder" /><p v-if="validationAttempted && !form.orderId" class="text-xs text-error">{{ $t("Choose a confirmed order before continuing.") }}</p><EmptyState v-if="!confirmedOrders.length" compact :title='$t("No confirmed orders available")' :description='$t("Confirm an order with at least one item before creating production work.")'><template #icon><ClipboardList :size="21" aria-hidden="true" /></template></EmptyState></template><div v-else class="grid min-w-0 gap-3 rounded-box border border-base-300 bg-base-100/45 p-4 sm:grid-cols-2"><div class="min-w-0"><span class="block text-xs text-base-content/55">{{ $t("Customer") }}</span><strong class="mt-1 block truncate text-sm">{{ customerLabel }}</strong></div><div class="min-w-0"><span class="block text-xs text-base-content/55">{{ $t("Order") }}</span><strong class="mt-1 block truncate text-sm">{{ props.workspace.jobOrder(selected!) }}</strong></div><div class="min-w-0 sm:col-span-2"><span class="block text-xs text-base-content/55">{{ $t("Service item") }}</span><strong class="mt-1 block truncate text-sm">{{ serviceLabel }}</strong></div></div></section>

            <section v-else-if="activeStep === 2 && isCreating" class="min-w-0 space-y-6"><div class="flex items-center gap-3 border-b border-base-300 pb-4"><span class="grid size-10 shrink-0 place-items-center rounded-box bg-primary/10 text-primary"><Package :size="21" aria-hidden="true" /></span><div><h2 class="text-lg font-semibold">{{ $t("Item and quantity") }}</h2><p class="text-sm text-base-content/60">{{ $t("Choose the service item and confirm the output quantity for this job.") }}</p></div></div><SelectField v-model="form.orderItemId" :label='$t("Order item")' :aria-label='$t("Order item")' :options="itemOptions" :invalid="validationAttempted && !form.orderItemId" @update:model-value="chooseItem" /><div class="grid min-w-0 gap-4 sm:grid-cols-2"><FormField :label='$t("Quantity")' required class="gap-1"><AppInput class="input w-full min-w-0" :class="{ 'input-error': validationAttempted && (!form.quantity || draftError.includes('quantity')) }" :model-value="formatQuantityInput(form.quantity)" required inputmode="decimal" placeholder="120" @update:model-value="form.quantity = formatQuantityInput($event)" /></FormField><FormField :label='$t("Unit")' required class="gap-1"><AppInput v-model="form.quantityUnit" class="input w-full min-w-0" :class="{ 'input-error': validationAttempted && (!form.quantityUnit.trim() || draftError.includes('unit')) }" required :placeholder='$t("piece")' /></FormField></div><p v-if="validationAttempted && draftError" class="text-xs text-error">{{ draftError }}</p></section>

            <section v-else-if="activeStep === 2 || (activeStep === 3 && isCreating)" class="min-w-0 space-y-6"><div class="flex items-center gap-3 border-b border-base-300 pb-4"><span class="grid size-10 shrink-0 place-items-center rounded-box bg-primary/10 text-primary"><CalendarClock :size="21" aria-hidden="true" /></span><div><h2 class="text-lg font-semibold">{{ $t("Schedule the work") }}</h2><p class="text-sm text-base-content/60">{{ $t("Assign the job and add the planning context operators need.") }}</p></div></div><div class="grid min-w-0 gap-4 sm:grid-cols-2"><SelectField v-model="form.assignedMachineId" :label='$t("Machine")' :options="[{ label: 'Unassigned', value: '' }, ...props.machines.map((machine) => ({ label: machine.name, value: machine.id }))]" /><SelectField v-model="form.priority" :label='$t("Priority")' :options="['Urgent', 'High', 'Normal', 'Low'].map((value) => ({ label: value, value }))" /><FormField :label='$t("Planned date")' class="gap-1 sm:col-span-2"><JalaliDatePicker v-model="form.plannedAt" :placeholder='$t("Set planned date")' /></FormField><FormField :label='$t("Notes")' class="gap-1 sm:col-span-2"><AppTextarea v-model="form.notes" class="textarea w-full min-w-0" rows="6" :placeholder='$t("Add production notes, handoff details, or quality instructions")' /></FormField></div></section>

            <section v-else class="min-w-0 space-y-6"><div class="flex items-center gap-3 border-b border-base-300 pb-4"><span class="grid size-10 shrink-0 place-items-center rounded-box bg-primary/10 text-primary"><FileText :size="21" aria-hidden="true" /></span><div><h2 class="text-lg font-semibold">{{ $t("Review and save") }}</h2><p class="text-sm text-base-content/60">{{ $t("Confirm the job context and schedule before") }} {{ $ui(isCreating ? 'creating' : 'saving') }}.</p></div></div><div class="rounded-box border border-base-300 bg-base-100/45 p-4"><dl class="divide-y divide-base-300/70 text-sm"><div class="flex justify-between gap-3 py-2"><dt class="text-base-content/60">{{ $t("Customer") }}</dt><dd class="max-w-[65%] truncate text-end">{{ customerLabel }}</dd></div><div class="flex justify-between gap-3 py-2"><dt class="text-base-content/60">{{ $t("Order") }}</dt><dd class="max-w-[65%] truncate text-end">{{ orderLabel }}</dd></div><div class="flex justify-between gap-3 py-2"><dt class="text-base-content/60">{{ $t("Service item") }}</dt><dd class="max-w-[65%] truncate text-end">{{ serviceLabel }}</dd></div><div class="flex justify-between gap-3 py-2"><dt class="text-base-content/60">{{ $t("Quantity") }}</dt><dd>{{ quantityLabel }}</dd></div><div class="flex justify-between gap-3 py-2"><dt class="text-base-content/60">{{ $t("Machine") }}</dt><dd class="max-w-[65%] truncate text-end">{{ machineLabel }}</dd></div><div class="flex justify-between gap-3 py-2"><dt class="text-base-content/60">{{ $t("Priority") }}</dt><dd>{{ $ui(form.priority) }}</dd></div><div class="flex justify-between gap-3 py-2 last:pb-0"><dt class="text-base-content/60">{{ $t("Planned date") }}</dt><dd>{{ plannedLabel }}</dd></div></dl></div><div v-if="isCreating && draftError" class="rounded-box border border-error/40 bg-error/10 p-3 text-sm text-error">{{ draftError }} {{ $t("Return to the earlier step to fix it.") }}</div></section>
          </form>
        </section>

        <aside class="service-wizard-preview-panel min-h-0 min-w-0 rounded-box border border-base-300 bg-base-200/35 p-4 xl:overflow-y-auto"><div class="space-y-4"><div class="relative min-h-48 overflow-hidden rounded-box bg-base-100 p-5"><div class="absolute inset-0 bg-gradient-to-br from-primary/10 via-transparent to-transparent" aria-hidden="true"></div><div class="relative flex min-h-36 flex-col justify-end"><span class="mb-auto grid size-11 place-items-center rounded-box border border-base-300 bg-base-200/80 text-primary"><Factory :size="24" aria-hidden="true" /></span><div class="mt-6 flex items-end justify-between gap-3"><div class="min-w-0"><h2 class="truncate text-lg font-semibold">{{ $ui(selected?.jobNumber || 'New production job') }}</h2><p class="mt-1 truncate text-xs text-base-content/60">{{ serviceLabel }}</p></div><StatusBadge :label="$ui(isCreating ? 'Pending' : (selected?.status || 'Pending'))" :tone="isCreating ? 'slate' : props.workspace.statusTone(selected?.status || 'Pending')" /></div></div></div><div class="divide-y divide-base-300 rounded-box border border-base-300 bg-base-100/35"><div class="flex items-center justify-between gap-3 px-3 py-3 text-sm"><span class="text-base-content/60">{{ $t("Customer") }}</span><strong class="max-w-[10rem] truncate text-end">{{ customerLabel }}</strong></div><div class="flex items-center justify-between gap-3 px-3 py-3 text-sm"><span class="text-base-content/60">{{ $t("Quantity") }}</span><strong>{{ quantityLabel }}</strong></div><div class="flex items-center justify-between gap-3 px-3 py-3 text-sm"><span class="text-base-content/60">{{ $t("Machine") }}</span><strong class="max-w-[10rem] truncate text-end">{{ machineLabel }}</strong></div><div class="flex items-center justify-between gap-3 px-3 py-3 text-sm"><span class="text-base-content/60">{{ $t("Priority") }}</span><strong>{{ $ui(form.priority) }}</strong></div></div><div class="flex gap-2 border-t border-base-300 pt-3 text-sm"><UserRound class="mt-0.5 shrink-0 text-info" :size="17" aria-hidden="true" /><p class="leading-5 text-base-content/70">{{ $ui(currentStep?.description) }}{{ $t(". The job stays linked to the confirmed order item after saving.") }}</p></div></div></aside>
      </div>
    </div>
    <footer class="flex min-w-0 items-center justify-between gap-3 border-t border-base-300 px-1 pt-3"><button class="btn btn-ghost btn-sm" type="button" :disabled="activeStep === 1 || busy || saving" @click="previous">{{ $t("Back") }}</button><span class="text-xs text-base-content/55">{{ $t("Step") }} {{ activeStep }} {{ $t("of") }} {{ steps.length }}</span><button v-if="activeStep < steps.length" class="btn btn-primary btn-sm" type="button" @click="next">{{ $t("Continue") }}</button><button v-else class="btn btn-success btn-sm gap-2" type="submit" form="production-editor-wizard" :disabled="busy || saving"><Save :size="14" aria-hidden="true" />{{ $ui(isCreating ? 'Create job' : 'Save changes') }}</button></footer>
  </div>
</template>
