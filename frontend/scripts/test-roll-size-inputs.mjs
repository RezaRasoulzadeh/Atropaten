...
const { ensureRollSizeInputs, materialOptionsForParameter, usesMaterialRollWidth, selectedRollMaterial, rollWidthValue, rollStockWidthValue } = await import(`data:text/javascript;base64,${Buffer.from(js).toString('base64')}`)
...
for (const category of ['Large format', 'Banners & signage', 'Paper printing']) {
  ...
  assert.deepEqual(materialOptionsForParameter(service, service.parameters.find(p => p.key === 'paper'), [material], {}), [{ value: '3200', label: 'Banner 320' }])
  assert.equal(rollWidthValue(material, '0'), '3200')
  assert.equal(rollWidthValue(material, '10'), '3180')
  assert.equal(rollStockWidthValue(material), '3200')
