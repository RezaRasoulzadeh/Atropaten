...
func (s *Store) syncOrderItemsTx(ctx context.Context, tx *sql.Tx, order domain.Order) error {
	...
	for _, i := range order.Items {
		...
		_, err = tx.ExecContext(ctx, `INSERT INTO order_items(id,order_id,display_order,service_id,service_name_snapshot,service_code_snapshot,quantity_units,quantity_unit,resolved_parameters_json,cost_breakdown_json,pricing_snapshot_json,estimated_cost_rial,suggested_price_rial,selling_price_rial,outsourced_cost_rial,outsourced_shipping_rial,outsourced_supplier_id,outsourced_notes,notes) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET display_order=excluded.display_order,service_id=excluded.service_id,service_name_snapshot=excluded.service_name_snapshot,service_code_snapshot=excluded.service_code_snapshot,quantity_units=excluded.quantity_units,quantity_unit=excluded.quantity_unit,resolved_parameters_json=excluded.resolved_parameters_json,cost_breakdown_json=excluded.cost_breakdown_json,pricing_snapshot_json=excluded.pricing_snapshot_json,estimated_cost_rial=excluded.estimated_cost_rial,suggested_price_rial=excluded.suggested_price_rial,selling_price_rial=excluded.selling_price_rial,outsourced_cost_rial=excluded.outsourced_cost_rial,outsourced_shipping_rial=excluded.outsourced_shipping_rial,outsourced_supplier_id=excluded.outsourced_supplier_id,outsourced_notes=excluded.outsourced_notes,notes=excluded.notes,removed_at=NULL WHERE order_items.order_id=excluded.order_id`, i.ID, order.ID, i.Position, i.ServiceID, i.ServiceNameSnapshot, i.ServiceCodeSnapshot, i.Quantity, i.QuantityUnit, i.ResolvedParametersJSON, i.CostBreakdownJSON, i.PricingSnapshotJSON, i.EstimatedCostRial, i.SuggestedPriceRial, i.SellingPriceRial, i.OutsourcedCostRial, i.OutsourcedShippingRial, nullableString(i.OutsourcedSupplierID), i.OutsourcedNotes, i.Notes)
		if err != nil {
			return err
		}
		if !exists {
